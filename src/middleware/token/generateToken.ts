...
export class Protected {
    static generateToken(c: Context, userId: string, role: string) {
        ...
        c.header(
            'Set-Cookie',
            `access_token=${token}; HttpOnly; Secure; Path=/; SameSite=Strict; Max-Age=${expiresIn}`,
        )
        return token;
    }
    ...
    static async generateRefreshToken(c: Context, userId: string) {
        ...
        c.header(
            'Set-Cookie',
            `refresh_token=${refresToken};HttpOnly; Secure; Path=/; SameSite=Strict; Max-Age=${expiresIn}`
        )
        return refresToken
    }
