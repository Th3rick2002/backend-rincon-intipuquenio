...
export class Protected {
    static generateToken(c: Context, userId: string, role: string) {
        ...
        c.header(
            'Set-Cookie',
            `access_token=${token}; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=${expireSeconds}`,
            { append: true }
        )
    ...
    static async generateRefreshToken(c: Context, userId: string) {
        ...
        c.header(
            'Set-Cookie',
            `refresh_token=${refresToken}; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=${refreshSeconds}`,
            { append: true }
        )
        return refresToken
    }
